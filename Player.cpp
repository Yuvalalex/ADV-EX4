...
void Player::resetLevelState() {
    // Reset physics and temporary states only (keeps inventory and score)
    resetToInitialPosition();
    currentDir = Direction::STAY;
...
ObjectType Player::handleDispose(Screen& screen, Player& otherPlayer) {
    // Dispose bomb
    if (_hasBomb) {
        // FIXED: Use smart pointer to prevent memory leak
