...
void PhysicsManager::movePlayer(Player& player, Player& otherPlayer,
    ...
    for (int step = 0; step < stepsToMove; ++step) {
        ...
        // Check 2: Player vs Player collision
        ...
        // Check 3: Spring compression logic
        if (player.getIsCompressing()) {
            bool isNextPositionSpring = (objectAtNext != nullptr &&
                objectAtNext->getType() == ObjectType::SPRING);
            if (!isNextPositionSpring) {
        ...
        // Check 4: Player vs Object interaction
